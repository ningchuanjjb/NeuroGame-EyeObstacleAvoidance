...
while exitDemo == false
    ...
    %meanSignal = mean(signal(end-200:end));
    meanSignal = mean(signal);            
%     if zeroBackFlag == 1
%         if abs(meanSignal) < saccadeThreshold
%             zeroBackFlag = 0;
%         end
%     end
...
%         if meanSignal < -1*saccadeThreshold
%             squareX = screenXpixels * squareX_leftBoundaryCoeff;
%         elseif meanSignal > saccadeThreshold
%             squareX = screenXpixels * squareX_rightBoundaryCoeff;
%         end
...
%     if abs(meanSignal) > 0.002 && signalFlag == 1
%         signal = 0;
%         signalFlag = 0;
%         t0_read_coldDown = tic; 
%         
